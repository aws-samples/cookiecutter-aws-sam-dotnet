...
* {{ cookiecutter.solution_name }}.sln - Visual Studio Solution file
* {{ cookiecutter.project_name }} - Code for the application's Lambda function and Project Dockerfile.
* {{ cookiecutter.project_name }}.Tests - Unit tests for the application code.
...
```bash
{{ cookiecutter.solution_name }}$ sam build
```
...
```bash
{{ cookiecutter.solution_name }}$ sam local invoke HelloWorldFunction --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
{{ cookiecutter.solution_name }}$ sam local start-api
{{ cookiecutter.solution_name }}$ curl http://localhost:3000/
```
...
```bash
{{ cookiecutter.solution_name }}$ sam logs -n HelloWorldFunction --stack-name Logging --tail
```
...
```bash
{{ cookiecutter.solution_name }}$ dotnet test
```
...
```bash
{{ cookiecutter.solution_name }}$ sam delete
```
